...
export default function AdminTeam() {
  const { draft, loading, saving, hasChanges, updateField, save, publish, revert } = usePageData('team');
  ...
  return (
    <div>
      ...
      <div className="bg-yellow-100 border-b-2 border-yellow-300 p-4 sticky top-16 z-40">
        <div className="container mx-auto flex justify-between items-center">
          <span className="text-yellow-800 font-semibold">
            ✏️ Edit Team Page - {hasChanges ? 'Unsaved changes' : 'All changes saved'}
          </span>
      ...
      <TeamGrid data={draft || {}} isEditable onChange={updateField} />
    </div>
  );
}